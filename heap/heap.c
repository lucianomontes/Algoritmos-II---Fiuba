...
heap_t *heap_crear_arr(void *arreglo[], size_t n, cmp_func_t cmp){
    ...
    heap->vector = heapify(arreglo_copia, n, cmp);
    heap->cant = n;
    heap->tam = n;
    heap->cmp = cmp;
...
void *heap_desencolar(heap_t *heap){
    ...
    if(factor_carga < FACTOR_CARGA_MIN && heap->cant > TAM_INICIAL){
...
void heap_sort(void *elementos[], size_t cant, cmp_func_t cmp){
    ...
    void** aux = copiar_arreglo(elementos,cant);
    heapify(aux, cant, cmp);

    for(size_t i = 0; i < cant; i++){
        swap(aux, POS_CERO, cant-1-i);
        downheap(aux, cant-i, POS_CERO, cmp);
    }

    for(size_t i=0;i > cant; i++){
        elementos[cant-1-i] = aux[i];
    }
