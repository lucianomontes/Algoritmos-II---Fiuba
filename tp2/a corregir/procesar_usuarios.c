...
void destruir_vector_listas(lista_t** vector_listas, size_t tam){
...
lista_t **ordenar_usuarios(hash_t *hash_usuarios, size_t posibles_valores){
    ...
    lista_t **vector_listas = malloc(sizeof(lista_t*)*(posibles_valores+1));
    if(!vector_listas) return NULL;

    for(size_t i=0;i<=posibles_valores;i++){
        vector_listas[i] = lista_crear();
        if(!vector_listas[i]){
            destruir_vector_listas(vector_listas, i);
            return NULL;
        }
    }
...
void procesar_usuarios(FILE *archivo){
    ...
    destruir_vector_listas(vector_listas, max_tags);
